...
namespace Sierra::Core::Rendering::Vulkan::Abstractions
{
    ...
    Pipeline::Pipeline(const PipelineCreateInfo &createInfo)
        : maxConcurrentFrames(createInfo.maxConcurrentFrames), usesPrecompiledShaders(createInfo.shaders[0]->IsPrecompiled()), descriptorSetCount(createInfo.shaderInfo.has_value() ? createInfo.shaderInfo->descriptorSetCount : 1)
    {
        ...
        if (usesPrecompiledShaders)
        {
            ...
            for (const auto &[bindingKey, bindingData] : shaderMembers)
            {
                ...
                else if (bindingData.memberType == ShaderMemberType::BUFFER)
                {
                    ...
                    bindingData.bufferData->buffers.resize(maxConcurrentFrames);
                }
    ...
    void Pipeline::Bind(const UniquePtr<CommandBuffer> &commandBuffer)
    {
        // Set bound status
        bound = true;
    ...
    void Pipeline::BindDescriptorSet(const UniquePtr<CommandBuffer> &commandBuffer, const UniquePtr<DescriptorSet> &givenDescriptorSet, const uint offset) const
    {
    ...
    void Pipeline::BindDescriptorSets(const UniquePtr<CommandBuffer> &commandBuffer, const std::vector<ReferenceWrapper<UniquePtr<DescriptorSet>>> &givenDescriptorSets, const uint offset) const
    {
        ...
        for (uint i = givenDescriptorSets.size(); i--;)
        {
    ...
    void Pipeline::SetShaderDefinition(const ShaderType shaderType, const ShaderDefinition definition)
    {
            ...
            if (!usesPrecompiledShaders)
            {
                ASSERT_WARNING_FORMATTED("Cannot set definition [{0}] for a shader that is within a pipeline, which uses compiled shaders! Returning", definition.name);
                return;
            }
        #endif
        ...
        auto iterator = shaders.find(shaderType);
            ...
            {
                ASSERT_WARNING_FORMATTED("Trying to set definition [{0}] for a shader of type [{1}], but no shader of such type is present in pipeline! Action suspended", definition.name, static_cast<uint>(shaderType));
                return;
            }
        #endif
    ...
    void Pipeline::SetSpecializationConstant(const uint constantID, const void* data, const uint range)
    {
        #if DEBUG
            if (bound)
            {
                ASSERT_WARNING_FORMATTED("Cannot set specialization constant [{0}] while pipeline is bound! Operation suspended", constantID);
                return;
            }
        #endif
        ...
        for (const auto &[shaderType, shader] : shaders)
        {
            ...
            for (uint i = shader->specializationInfo->mapEntryCount; i--;)
            {
    ...
    void Pipeline::SetShaderMember(const String &memberName, const UniquePtr<Image> &image, const UniquePtr<Sampler> &sampler, const uint arrayIndex, ImageLayout imageLayoutAtDrawTime)
    {
        ...
        ShaderMember &member = GetShaderMember(memberName);
        ASSERT_ERROR_FORMATTED_IF(member.memberType != ShaderMemberType::TEXTURE, "Trying to set image and sampler for member [{0}], but it is not of type ShaderMemberType::TEXTURE", memberName);
    ...
    void Pipeline::SetShaderMember(const String &memberName, const SharedPtr<Texture> &texture, const uint arrayIndex)
    {
        ...
        ShaderMember &member = GetShaderMember(memberName);
        ASSERT_ERROR_FORMATTED_IF(member.memberType != ShaderMemberType::TEXTURE, "Trying to set texture for member [{0}], but it is not of type ShaderMemberType::TEXTURE", memberName);
    ...
    void Pipeline::SetShaderMember(const String &memberName, const UniquePtr<Cubemap> &cubemap, const uint arrayIndex)
    {
        ...
        ShaderMember &member = GetShaderMember(memberName);
        ASSERT_ERROR_FORMATTED_IF(member.memberType != ShaderMemberType::CUBEMAP, "Trying to set cubemap for member [{0}], but it is not of type ShaderMemberType::CUBEMAP", memberName);
    ...
    void Pipeline::SetShaderMember(const String &memberName, void *data, const uint range, const uint offset)
    {
        ...
        ASSERT_ERROR_FORMATTED_IF(bound && member.memberType != ShaderMemberType::PUSH_CONSTANT, "Cannot modify non-push-constant shader member data with this method during drawing/dispatch! To set shader members to a bound pipeline you can get the binding for the member [{0}] and create a dedicated descriptor set", memberName);
        ...
        switch (member.memberType)
        {
            ...
            default:
            {
                ASSERT_ERROR_FORMATTED("Cannot set plain pointer data for member [{0}], as it is neither of type ShaderMemberType::BUFFER, nor of ShaderMemberType::PUSH_CONSTANT", memberName);
            }
        }
    }
    ...
    std::unordered_set<uint> Pipeline::GetShaderMemberBindings(const String &memberName)
    {
        ShaderMember &member = GetShaderMember(memberName);
        ASSERT_ERROR_FORMATTED_IF(member.memberType == ShaderMemberType::PUSH_CONSTANT, "Cannot get the binding index of member [{0}], as it is of type ShaderMemberType::PUSH_CONSTANT", memberName);
