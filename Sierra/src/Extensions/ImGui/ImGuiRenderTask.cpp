...
namespace Sierra
{
    ...
    constexpr float64 VERTEX_BUFFER_GROWTH_MULTIPLIER = 1.8f;
    constexpr float64 INDEX_BUFFER_GROWTH_MULTIPLIER = VERTEX_BUFFER_GROWTH_MULTIPLIER * 3.6f;
    ...
    void ImGuiRenderTask::Render(std::unique_ptr<CommandBuffer> &commandBuffer, const std::unique_ptr<Image> &image)
    {
        ...
        if (const uint64 requiredVertexMemorySize = drawData->TotalVtxCount * sizeof(ImDrawVert); requiredVertexMemorySize > vertexBuffer->GetMemorySize())
        {
            ...
            std::unique_ptr<Sierra::Buffer> newVertexBuffer = renderingContext.CreateBuffer({
                .name = "Vertex Buffer of ImGui Render Task",
                .memorySize = glm::max(requiredVertexMemorySize, static_cast<uint64>(static_cast<float64>(vertexBuffer->GetMemorySize()) * VERTEX_BUFFER_GROWTH_MULTIPLIER)),
                .usage = BufferUsage::Vertex,
                .memoryLocation = BufferMemoryLocation::CPU
            });
        ...
        {
            // Create new buffer with bigger memory
            std::unique_ptr<Sierra::Buffer> newIndexBuffer = renderingContext.CreateBuffer({
                .name = "Index Buffer of ImGui Render Task",
                .memorySize = glm::max(requiredIndexMemorySize, static_cast<uint64>(static_cast<float64>(indexBuffer->GetMemorySize()) * INDEX_BUFFER_GROWTH_MULTIPLIER)),
                .usage = BufferUsage::Index,
                .memoryLocation = BufferMemoryLocation::CPU
            });
        ...
        for (int i = 0; i < drawData->CmdListsCount; i++)
        {
            ...
            vertexBuffer->CopyFromMemory(commandList->VtxBuffer.Data, commandList->VtxBuffer.Size * sizeof(ImDrawVert), 0, vertexOffset * sizeof(ImDrawVert));
            indexBuffer->CopyFromMemory(commandList->IdxBuffer.Data, commandList->IdxBuffer.Size * sizeof(ImDrawIdx), 0, indexOffset * sizeof(ImDrawIdx));
