...
namespace Sierra
{
    ...
    void FoundationFileStream::Seek(const size offset)
    {
        ...
        if (error != nil) HandleNSFileError(error, SR_FORMAT("Could not seek to offset [{0}]", offset), filePath);
    }

    std::vector<uint8> FoundationFileStream::Read(const size memorySize)
    {
        SR_THROW_IF(memorySize > GetMemorySize(), InvalidFileRange("Cannot read from file", GetFilePath(), GetCurrentOffset(), memorySize, GetMemorySize()));

        NSError* error = nil;
        NSData* const data = [fileHandle readDataUpToLength: memorySize error: &error];

        if (error != nil) HandleNSFileError(error, SR_FORMAT("Could not read [{0}] bytes", memorySize), filePath);
    ...
    void FoundationFileStream::Write(const void* memory, const size sourceOffset, const size memorySize)
    {
        ...
        if (error != nil) HandleNSFileError(error, SR_FORMAT("Could not write [{0}] bytes", memorySize), filePath);
    }
