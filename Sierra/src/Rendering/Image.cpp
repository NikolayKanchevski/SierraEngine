...
namespace Sierra
{
    ...
    {
        return static_cast<float32>(ImageFormatToBlockMemorySize(format)) / static_cast<float32>(ImageFormatToBlockSize(format));
    }
    ...
    uint8 ImageFormatToBlockMemorySize(const ImageFormat format)
    {
        switch (format)
        {
            case ImageFormat::Undefined:                break;
            ...
            case ImageFormat::BC1_RGBA_UNorm:
            case ImageFormat::BC1_RGBA_SRGB:
            case ImageFormat::BC3_RGBA_UNorm:       return 2;
            case ImageFormat::BC3_RGBA_SRGB:
            case ImageFormat::BC4_R_Norm:           return 4;
            case ImageFormat::BC4_R_UNorm:
            case ImageFormat::BC5_RG_Norm:          return 2;
            case ImageFormat::BC5_RG_UNorm:
            case ImageFormat::BC6_HDR_RGB_Float:
            case ImageFormat::BC6_HDR_RGB_UFloat:
            case ImageFormat::BC7_RGB_UNorm:        return 4;
            case ImageFormat::BC7_RGB_SRGB:
            case ImageFormat::BC7_RGBA_UNorm:
    ...
    uint64 Image::GetMemorySize() const noexcept
    {
        const ImageFormat format = GetFormat();
        const uint8 blockSize = ImageFormatToBlockSize(format);
        const uint8 blockMemorySize = ImageFormatToBlockMemorySize(format);
        return glm::max(1U, (GetWidth() / blockSize)) * glm::max(1U, (GetHeight() / blockSize)) * glm::max(1U, (GetDepth() / blockSize)) * blockMemorySize;
    }
