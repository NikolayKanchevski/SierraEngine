...
namespace Sierra
{
    ...
    MTLCullMode CullModeToCullMode(const CullMode cullMode) noexcept
    {
        switch (cullMode)
        {
    ...
    MTLTriangleFillMode ShadeModeToTriangleFillMode(const ShadeMode shadeMode) noexcept
    {
        switch (shadeMode)
        {
    ...
    MTLWinding FrontFaceModeToWinding(const FrontFaceMode frontFaceMode) noexcept
    {
        ...
        : GraphicsPipeline(createInfo), cullMode(CullModeToCullMode(createInfo.cullMode)), triangleFillMode(ShadeModeToTriangleFillMode(createInfo.shadeMode)), winding(FrontFaceModeToWinding(createInfo.frontFaceMode))
    {
        ...
        SR_THROW_IF(createInfo.templateRenderPass.GetBackendType() != RenderingBackendType::Metal, UnexpectedTypeError(SR_FORMAT("Cannot create graphics pipeline [{0}] with template render pass [{1}], as its backend type differs from [RenderingBackendType::Metal]", createInfo.name, createInfo.templateRenderPass.GetName())));
        ...
        {
            SR_THROW_IF(createInfo.fragmentShader->GetBackendType() != RenderingBackendType::Metal, UnexpectedTypeError(SR_FORMAT("Cannot create graphics pipeline [{0}] with fragment shader [{1}], as its backend type differs from [RenderingBackendType::Metal]", createInfo.name, createInfo.fragmentShader->GetName())));
        ...
        // Set sample count
        [renderPipelineDescriptor setRasterSampleCount: ImageSamplingToUInteger(createInfo.sampling)];
        ...
        MTLVertexDescriptor* const vertexDescriptor = [MTLVertexDescriptor vertexDescriptor];
        ...
        if (metalRenderPass.HasDepthAttachment())
        {
            ...
            device.SetResourceName(depthStencilDescriptor, SR_FORMAT("Depth stencil state of graphics pipeline [{0}]", GetName()));
        ...
        SR_THROW_IF(error != nil, UnknownDeviceError(SR_FORMAT("Could not create graphics pipeline [{0}]", createInfo.name)));
    ...
    std::string_view MetalGraphicsPipeline::GetName() const noexcept
    {
