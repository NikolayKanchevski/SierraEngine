...
namespace Sierra
{
    ...
    MetalSwapchain::MetalSwapchain(const MetalDevice& givenDevice, const SwapchainCreateInfo& createInfo)
        : MetalResource(createInfo.name), Swapchain(createInfo), device(&givenDevice), window(&createInfo.window)
    {
        #if SR_PLATFORM_macOS
            SR_THROW_IF(window->GetBackendType() != WindowingBackendType::Cocoa, UnexpectedTypeError(SR_FORMAT("Cannot create swapchain [{0}] using window [{1}], as its platform backend differs from [PlatformBackendType::Cocoa]", GetName(), window->GetTitle())));
        ...
        #elif SR_PLATFORM_iOS
            SR_THROW_IF(window->GetBackendType() != WindowingBackendType::UIKit, UnexpectedTypeError(SR_FORMAT("Cannot create swapchain [{0}] using window [{1}], as its platform backend differs from [PlatformBackendType::UIKit]", GetName(), window->GetTitle())));
        ...
        // Handle resizing
        windowResizeEventListenerID = window->AddEventListener<WindowResizeEvent>([this](const WindowResizeEvent&) -> bool
        {
            // Resize Metal layer
            [metalLayer setDrawableSize: CGSizeMake(window->GetFramebufferWidth(), window->GetFramebufferHeight())];
    ...
    {
        SR_THROW_IF(commandBuffer.GetBackendType() != RenderingBackendType::Metal, UnexpectedTypeError(SR_FORMAT("Cannot present swapchain [{0}] using command buffer [{1}], as its backend type differs from [RenderingBackendType::Metal]", GetName(), commandBuffer.GetName())));
        ...
        device->SetResourceName(presentationCommandBuffer, SR_FORMAT("Presentation command buffer of swapchain [{0}]", GetName()));
    ...
    void MetalSwapchain::Recreate()
    {
        ...
        swapchainImage = MetalImage(*device, {
            .name = SR_FORMAT("Image of swapchain [{0}]", GetName()),
