...
namespace Sierra
{
    ...
    void MetalCommandBuffer::PushConstants(const void* memory, const uint16 memoryByteSize, const uint16 byteOffset)
    {
        ...
        {
            [currentComputeEncoder setBytes: reinterpret_cast<const uint8*>(memory) + byteOffset length: memoryByteSize atIndex: MetalDevice::PUSH_CONSTANT_INDEX];
        }
        else
        {
            [currentRenderEncoder setVertexBytes: reinterpret_cast<const uint8*>(memory) + byteOffset length: memoryByteSize atIndex: MetalDevice::PUSH_CONSTANT_INDEX];
            if (currentGraphicsPipeline->HasFragmentShader()) [currentRenderEncoder setFragmentBytes: reinterpret_cast<const uint8*>(memory) + byteOffset length: memoryByteSize atIndex: MetalDevice::PUSH_CONSTANT_INDEX];
        }
    }
