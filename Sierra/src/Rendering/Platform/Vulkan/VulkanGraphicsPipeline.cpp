...
namespace Sierra
{
    ...
    VulkanGraphicsPipeline::VulkanGraphicsPipeline(const VulkanDevice &device, const GraphicsPipelineCreateInfo &createInfo)
        : GraphicsPipeline(createInfo), VulkanResource(createInfo.name), device(device)
    {
        ...
        VkResult result = device.GetFunctionTable().vkCreatePipelineLayout(device.GetLogicalDevice(), &layoutCreateInfo, nullptr, &pipelineLayout);
        SR_ERROR_IF(result != VK_SUCCESS, "[Vulkan]: Could not create pipeline layout for graphics pipeline [{0}]! Error code: {1}.", GetName(), result);
        ...
        for (uint32 i = 0; i < createInfo.vertexInputs.size(); i++)
        {
            ...
            vertexInputAttributes[i].offset = nextVertexInputOffset;
        ...
        vertexInputBinding.stride = nextVertexInputOffset;
        ...
        // Create pipeline
        result = device.GetFunctionTable().vkCreateGraphicsPipelines(device.GetLogicalDevice(), nullptr, 1, &graphicsPipelineCreateInfo, nullptr, &pipeline);
        SR_ERROR_IF(result != VK_SUCCESS, "[Vulkan]: Could not create graphics pipeline [{0}]! Error code: {1}.");
    }

    /* --- POLLING METHODS --- */

    void VulkanGraphicsPipeline::Begin(std::unique_ptr<CommandBuffer> &commandBuffer) const
    {
        ...
        // Bind pipeline
        device.GetFunctionTable().vkCmdBindPipeline(vulkanCommandBuffer.GetVulkanCommandBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);
    }

    void VulkanGraphicsPipeline::End(std::unique_ptr<CommandBuffer> &commandBuffer) const
    {
    ...
    {
        SR_ERROR_IF(commandBuffer->GetAPI() != GraphicsAPI::Vulkan, "[Vulkan]: Cannot draw graphics pipeline [{0}] using command buffer [{1}], as its graphics API differs from [GraphicsAPI::Vulkan]!", GetName(), commandBuffer->GetName());
        const VulkanCommandBuffer &vulkanCommandBuffer = static_cast<const VulkanCommandBuffer&>(*commandBuffer);
