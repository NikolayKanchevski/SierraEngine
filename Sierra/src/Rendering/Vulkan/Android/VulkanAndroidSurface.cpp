...
namespace Sierra
{
    ...
    VkSurfaceKHR VulkanAndroidSurface::Create(const VulkanInstance& instance, const Window& window)
    {
        SR_ERROR_IF(window.GetBackendType() != WindowingBackendType::GameActivity, "[Vulkan]: Cannot create a Android surface for Vulkan using window [{0}], which has a platform backend, that differs from from [PlatformBackendType::GameActivity]", window.GetTitle());
