...
namespace Sierra
{
    ...
    public:
        /* --- CONSTRUCTORS --- */
        explicit RNG(uint64 seed = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count()) noexcept;

        /* --- SETTER METHODS --- */
        void SetSeed(uint64 seed) noexcept;
        ...
        /* --- COPY SEMANTICS --- */
        RNG(const RNG&) noexcept = default;
        RNG& operator=(const RNG&) noexcept = default;
