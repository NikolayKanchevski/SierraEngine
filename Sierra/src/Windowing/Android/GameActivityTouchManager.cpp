        ...
        for (size i = 0; i < inputBuffer->motionEventsCount; i++)
        {
            ...
            switch (motionEvent.action)
            {
                case AMOTION_EVENT_ACTION_DOWN:
                {
                    ...
                    const auto iterator = std::ranges::find(activeTouches, touch);
                ...
                case AMOTION_EVENT_ACTION_MOVE:
                {
                    ...
                    const auto iterator = std::ranges::find_if(activeTouches, [&pointer](const Touch& item) { return uintptr_t(item.GetID()) == pointer.id; });
