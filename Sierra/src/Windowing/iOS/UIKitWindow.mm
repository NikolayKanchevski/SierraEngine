...
@implementation UIKitWindowViewController
    ...
    {
        switch (window->GetAllowedOrientations())
        {
            case Sierra::ScreenOrientation::Portrait:               return UIInterfaceOrientationPortrait;
            case Sierra::ScreenOrientation::PortraitFlipped:        return UIInterfaceOrientationPortraitUpsideDown;
            case Sierra::ScreenOrientation::PortraitAny:            return UIInterfaceOrientationMaskPortrait;
            case Sierra::ScreenOrientation::Landscape:              return UIInterfaceOrientationLandscapeLeft;
            case Sierra::ScreenOrientation::LandscapeFlipped:       return UIInterfaceOrientationLandscapeRight;
            case Sierra::ScreenOrientation::LandscapeAny:           return UIInterfaceOrientationMaskLandscape;
            default:                                                break;
        }
    ...
    - (void) touchesBegan: (NSSet<UITouch*>*) touches withEvent: (UIEvent*) event
    {
        ...
        for (const UITouch* rawTouch in touches)
        {
            ...
            const Vector2 position = { static_cast<float32>([rawTouch locationInView: rawTouch.window.rootViewController.view].x), static_cast<float32>([UIScreen mainScreen].bounds.size.height) - static_cast<float32>([rawTouch locationInView: rawTouch.window.rootViewController.view].y) };
...
namespace Sierra
{
    ...
    std::string_view UIKitWindow::GetTitle() const noexcept
    {
        return title;
    }

    Vector2Int UIKitWindow::GetPosition() const noexcept
    {
        return { 0, 0 };
    }
    ...
    uint32 UIKitWindow::GetFramebufferHeight() const noexcept
    {
        return GetHeight() * static_cast<uint32>([window contentScaleFactor]);
    }

    float32 UIKitWindow::GetOpacity() const noexcept
    {
        return 1.0f;
    }

    bool UIKitWindow::IsClosed() const noexcept
    {
        return closed;
    }

    bool UIKitWindow::IsMinimized() const noexcept
    {
        return minimized;
    }

    bool UIKitWindow::IsMaximized() const noexcept
    {
        return !minimized;
    }

    bool UIKitWindow::IsFocused() const noexcept
    {
        return !minimized;
    }

    bool UIKitWindow::IsHidden() const noexcept
    {
        return minimized;
    }
    ...
    UIKitWindow::~UIKitWindow() noexcept
    {
        if (closed) return;
        closed = true;
