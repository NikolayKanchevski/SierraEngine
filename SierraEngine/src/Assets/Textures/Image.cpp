...
namespace SierraEngine
{
    ...
    Image::Image(const ImageCreateInfo& createInfo)
        : width(createInfo.width), height(createInfo.height), format(createInfo.format)
    {
        APP_THROW_IF(width == 0, Sierra::InvalidValueError("Cannot create image, as specified width must not be [0]!"));
        APP_THROW_IF(height == 0, Sierra::InvalidValueError("Cannot create image, as specified height must not be [0]!"));
        APP_THROW_IF(format == Sierra::ImageFormat::Undefined, Sierra::InvalidValueError("Cannot create image, as specified format must not be [ImageFormat::Undefined]!"));
        APP_THROW_IF(ImageFormatToBlockSize(format) > 1, Sierra::InvalidValueError("Cannot create image, as specified format must not be compressed!"));

        if (const size memorySize = width * height * ImageFormatToBlockMemorySize(format); !createInfo.memory.empty())
        {
            APP_THROW_IF(createInfo.memory.size() != memorySize, Sierra::InvalidValueError(SR_FORMAT("Cannot create image, as size of specified memory must be [{0} = width * height * ImageFormatToPixelMemorySize(format)]!", memorySize)));
            memory = std::move(createInfo.memory);
        }
        else
        {
            memory = std::vector<uint8>(memorySize);
        }
    }

    /* --- POLLING METHODS --- */

    std::vector<uint8> Image::Release() noexcept
    {
        width = 0;
        height = 0;
        format = Sierra::ImageFormat::Undefined;
        return std::move(memory);
    }

    /* --- SETTER METHODS --- */

    void Image::SetPixel(const Vector2UInt coordinate, const Color64 color)
    {
        APP_THROW_IF(coordinate.x >= width, Sierra::ValueOutOfRangeError("Cannot set pixel at invalid position at the horizontal axis of image", coordinate.x, 0U, width));
        APP_THROW_IF(coordinate.y >= height, Sierra::ValueOutOfRangeError("Cannot set pixel at invalid position at the vertical axis of image", coordinate.y, 0U, height));
        ...
        void* pixel = memory.data() + (coordinate.y * width + coordinate.x) * channelCount * channelMemorySize;
        switch (format)
        {
        ...
        APP_WARNING("Could not set pixel [{0}, {1}] of image", coordinate.x, coordinate.y);
    }
    
    /* --- GETTER METHODS --- */

    Color64 Image::GetPixel(const Vector2UInt coordinate) const
    {
        APP_THROW_IF(coordinate.x >= width, Sierra::ValueOutOfRangeError("Cannot get pixel at invalid position at the horizontal axis of image", coordinate.x, 0U, width));
        APP_THROW_IF(coordinate.y >= height, Sierra::ValueOutOfRangeError("Cannot get pixel at invalid position at the vertical axis of image", coordinate.y, 0U, height));

        const uint8 channelCount = Sierra::ImageFormatToChannelCount(format);
        const uint8 channelMemorySize = static_cast<uint8>(Sierra::ImageFormatToChannelMemorySize(format));

        const void* pixel = memory.data() + (coordinate.y * width + coordinate.x) * channelCount * channelMemorySize;
        switch (format)
        {
            case Sierra::ImageFormat::R8_Int:
            case Sierra::ImageFormat::R8G8_Int:
        ...
        APP_WARNING("Could not get pixel [{0}, {1}] of image", coordinate.x, coordinate.y);
        return { };
    }
