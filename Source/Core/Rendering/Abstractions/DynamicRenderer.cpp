...
namespace Sierra::Rendering
{
    ...
    {
        ASSERT_ERROR_IF(!VK::GetDevice()->IsExtensionLoaded(VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME), "Cannot create dynamic renderer, unless extension [{0}] is supported and loaded", VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME);
        ...
        uint32 biggestWidth = 0, biggestHeight = 0, colorAttachmentCount = createInfo.attachments.size() - (uint)(hasDepthAttachment);
        ...
        for (const auto &attachmentInfo : createInfo.attachments)
        {
            ...
            if (attachmentInfo.image->GetWidth() > biggestWidth) biggestWidth = static_cast<uint32>(attachmentInfo.image->GetWidth());
            if (attachmentInfo.image->GetHeight() > biggestHeight) biggestHeight = static_cast<uint32>(attachmentInfo.image->GetHeight());
        }
    ...
    void DynamicRenderer::OverloadColorAttachment(const uint32 index, UniquePtr<Image> &image)
    {
