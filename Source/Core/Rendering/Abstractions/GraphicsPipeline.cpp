...
namespace Sierra::Rendering
{
    ...
    void GraphicsPipeline::CreatePipeline()
    {
            ...
            if (shaderType == ShaderType::VERTEX)
            {
                if (usesPrecompiledShaders)
                {
                    ...
                    if (reflectionData.vertexAttributes != nullptr)
                    {
                        ...
                        for (uint32 j = 0; j < attributeDescriptions.size(); j++)
                        {
        ...
        if (sampling > Sampling::MSAAx1)
        {
            ...
            {
                ASSERT_WARNING("Requested a sampling of [{0}] for pipeline, however, highest sampling supported is [{1}]! Sampling was automatically lowered to maximum", VK_TO_STRING(sampling, SampleCountFlagBits), VK_TO_STRING(VK::GetDevice()->GetHighestMultisampling(), SampleCountFlagBits));
        ...
        uint32 colorAttachmentCount = usesRenderPasses ? renderPassInfo.value().renderPass->GetColorAttachmentCount(renderPassInfo.value().subpass) : dynamicRenderingInfo.value().dynamicRenderer->GetColorAttachmentCount();
    ...
    void GraphicsPipeline::Draw(const UniquePtr<CommandBuffer> &commandBuffer, const uint32 vertexCount)
    {
        BindResources(commandBuffer);
        vkCmdDraw(commandBuffer->GetVulkanCommandBuffer(), vertexCount, 1, 0, 0);
    }

    void GraphicsPipeline::DrawMesh(const UniquePtr<CommandBuffer> &commandBuffer, const UniquePtr<Engine::Mesh> &mesh)
    {
        BindResources(commandBuffer);
