...
namespace Sierra::Rendering
{
    ...
    RenderPass::RenderPass(const RenderPassCreateInfo &createInfo)
    {
        ...
        uint32 totalColorAttachmentLoadCount = 0;
        ...
        for (uint32 i = createInfo.attachments.size(); i--;)
        {
        ...
        for (uint32 i = createInfo.subpassInfos.size(); i--;)
        {
            // Set default values
            ...
            if (subpassInfo.loadDepthAttachment && depthAttachmentIndex != -1)
            {
                subpassDescription.SetDepthReference({
                    .attachment = static_cast<uint32>(depthAttachmentIndex),
            ...
            for (uint32 j = 0; j < subpassInfo.renderTargets.size(); j++)
            {
                uint32 renderTargetIndex = subpassInfo.renderTargets[j];
                switch (createInfo.attachments[renderTargetIndex].type)
                {
                    case RenderPassAttachmentType::SWAPCHAIN:
                    {
                        ...
                        subpasses[i].writesToSwapchainImage = true;
                        break;
                    }
                    case RenderPassAttachmentType::COLOR:
                    {
            ...
            for (uint32 j = 0; j < subpassInfo.subpassInputs.size(); j++)
            {
                uint32 subpassInputIndex = subpassInfo.subpassInputs[j];
            ...
            // Add special dependency for swapchain synchronization
            if (subpasses[i].writesToSwapchainImage)
            {
                subpassDependencies.push_back({
            ...
            // Add special dependency between current and previous subpass to synchronize depth attachment usage
        ...
        // Set last dependency
        subpassDependencies.push_back({
            .srcSubpass = static_cast<uint32>(createInfo.subpassInfos.size() - 1),
        ...
        renderPassCreateInfo.subpassCount = static_cast<uint32>(vkSubpassDescriptions.size());
        ...
        VK_ASSERT(vkCreateRenderPass(VK::GetLogicalDevice(), &renderPassCreateInfo, nullptr, &vkRenderPass), "Could not create render pass");
    }
    ...
    void RenderPass::SetClearColor(const uint32 colorAttachmentIndex, const Vector4& color)
    {
    ...
    {
        vkDestroyRenderPass(VK::GetLogicalDevice(), vkRenderPass, nullptr);
