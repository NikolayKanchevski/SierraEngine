...
namespace Sierra::Rendering
{
        ...
        : filePath(createInfo.filePath), shaderType(createInfo.shaderType != ShaderType::NONE ? createInfo.shaderType : GetShaderTypeFromExtension())
    {
        ...
        ASSERT_ERROR_IF(result != SPV_REFLECT_RESULT_SUCCESS, "Could not create reflection module for shader [{0}]", filePath.c_str());
        ...
        : filePath(createInfo.filePath), shaderType(createInfo.shaderType)
    {
        ...
        if (!createInfo.vertexAttributes.empty())
        {
            ...
            // Populate attributes
            uint32 lastOffset = 0;
            for (uint32 i = 0; i < createInfo.vertexAttributes.size(); i++)
            {
    ...
    std::vector<uint32> Shader::CompileShadercShader()
    {
        // Set options
        ...
        uint32 startingBindingForStage = MAX_BINDINGS_PER_STAGE * GetShadercShaderType();
        ...
        shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(shaderCode.data(), shaderCode.size(), GetShadercShaderType(), filePath.stem().c_str(), options);

        // Check for errors
        if (module.GetCompilationStatus() != shaderc_compilation_status_success)
        {
            ASSERT_ERROR("Precompiling shader [{0}] failed with an error code of [{1}]:\n\t{2}", filePath.c_str(), static_cast<uint32>(module.GetCompilationStatus()), module.GetErrorMessage());
        }
    ...
    void Shader::SaveReflectionData(const SpvReflectShaderModule &reflectionModule)
    {
        ...
        if (shaderType == ShaderType::VERTEX && reflectionModule.input_variable_count > 0)
        {
            ...
            for (uint32 i = 0; i < reflectionModule.input_variable_count; i++)
            {
        ...
        for (uint32 i = 0; i < reflectionModule.descriptor_binding_count; i++)
        {
            ...
            auto &descriptorBindingOutputData = (*precompiledData->reflectionData.members)[{
                    ...
                    .typeName = descriptorBinding.type_description->type_name != nullptr ? String(descriptorBinding.type_description->type_name) : std::to_string(static_cast<uint32>(descriptorBinding.descriptor_type)),
                #endif
            ...
            for (uint32 j = descriptorBinding.array.dims_count; j--;)
            {
            ...
            switch (descriptorBinding.descriptor_type)
            {
                ...
                case SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_BUFFER:
                {
                    ...
                    for (uint32 j = 0; j < descriptorBinding.block.member_count; j++)
                    {
                ...
                default:
                {
                    ASSERT_ERROR("Variables of type [{0}] are not supported in precompiled shaders", static_cast<uint32>(descriptorBinding.descriptor_type));
                    break;
                }
            }
        }
        ...
        {
            ASSERT_ERROR("Shader [{0}] has {1} push constant blocks! Vulkan only allows one", filePath.c_str(), reflectionModule.push_constant_block_count);
        }
        else
        {
            if (reflectionModule.push_constant_block_count != 0)
            {
                ...
                ASSERT_ERROR_IF(pushConstantBlock.size > 128, "Vulkan requires all push constants to be of a size no bigger than 128 bytes! This shader's push constant has a size of {0} bytes", pushConstantBlock.size);
        ...
        if (reflectionModule.specialization_constant_count != 0)
        {
            ...
            for (uint32 i = reflectionModule.specialization_constant_count; i--;)
            {
        ...
        uint32 lastOffset = 0;
        for (uint32 i = 0; i < reflectionModule.specialization_constant_count; i++)
        {
    ...
    {
        if (strcmp(filePath.extension().c_str(), ".vert") == 0) return ShaderType::VERTEX;
        if (strcmp(filePath.extension().c_str(), ".frag") == 0) return ShaderType::FRAGMENT;
        if (strcmp(filePath.extension().c_str(), ".geom") == 0) return ShaderType::GEOMETRY;
        if (strcmp(filePath.extension().c_str(), ".tese") == 0) return ShaderType::TESSELATION_EVALUATION;
        if (strcmp(filePath.extension().c_str(), ".tesc") == 0) return ShaderType::TESSELATION_CONTROL;
        if (strcmp(filePath.extension().c_str(), ".comp") == 0) return ShaderType::COMPUTE;
        if (strcmp(filePath.extension().c_str(), ".glsl") == 0) ASSERT_ERROR("Cannot determine the type of a shader file with extension of [.glsl]");
    ...
    template<typename T, std::enable_if_t<std::is_same_v<T, uint32> || std::is_same_v<T, char>, bool>>
    void Shader::CreateShaderModule(const char* entryPoint, const std::vector<T> &code)
    {
        ...
        VK_ASSERT(
            ...
            FORMAT_STRING("Failed to create shader module for [{0}]", filePath.c_str())
        );
    ...
    shaderc_include_result* Shader::Includer::GetInclude(const char* requestedSource, const shaderc_include_type type, const char* requestingSource, const uSize includeDepth)
    {
        ...
        // Gather its data
        auto fileData = Engine::File::ReadBinaryFile(filePath.parent_path() / includeReference);
