...
namespace Sierra::Rendering
{
    ...
    struct ShaderMemberBlock
    {
        ...
        [[nodiscard]] inline uint32 GetStride() const
        {
    ...
    struct ShaderDescriptorData
    {
        ...
        std::unordered_set<uint32> bindings;
        uint32 arraySize = 1;
        DescriptorType type;
    };
    ...
    private:
        ...
        std::vector<uint32> CompileShadercShader();
        template<typename T, ENABLE_IF(std::is_same_v<T, uint32> || std::is_same_v<T, char>)>
    ...
    private:
        ...
        inline static constexpr uint32 MAX_BINDINGS_PER_SHADER = 65535;
        inline static constexpr uint32 MAX_BINDINGS_PER_STAGE = MAX_BINDINGS_PER_SHADER / (shaderc_tess_evaluation_shader + 1);
