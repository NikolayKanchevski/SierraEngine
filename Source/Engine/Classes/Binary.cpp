//
// Created by Nikolay Kanchevski on 6.11.22.
//

#include "Binary.h"

namespace Sierra::Engine
{

    /* --- CONSTRUCTORS --- */

    Binary::Binary(uint32 givenValue)
        : value(givenValue)
    {

    }

    /* --- SETTER METHODS --- */

    void Binary::SetBit(uint32 bitIndex, bool newValue)
    {
        newValue != 0 ? static_cast<void>(value |= (1 << bitIndex)) : static_cast<void>(value &= ~(1 << bitIndex));
    }

    /* --- GETTER METHODS --- */

    uint32 Binary::GetBitCount(Binary binary)
    {
        uSize counter = 0;
        for (uSize i = 0; i < 8 * UINT_SIZE; i++) counter += static_cast<bool>(binary.value & (1 << i));
        return counter;
    }

    /* --- OPERATORS --- */

    Binary &Binary::operator=(uint32 givenValue)
    {
        value = givenValue;
        return *this;
    }
}