...
namespace Sierra::Engine
{
    ...
    void Discord::Activity::Clear(const std::function<void()> &OnActivityClearedCallback)
    {
        core->ActivityManager().ClearActivity([](const discord::Result result){
            DC_VALIDATE(result, FORMAT_STRING("Clearing Discord activity failed with error code: {0}", static_cast<uint32>(result)));
        });
    ...
    void Discord::Start(const uint64 applicationID, const Callback &OnUserLoadedCallback)
    {
        ...
        {
            ASSERT_WARNING("Could not create Discord instance! Error code: {0}", static_cast<uint32>(result));
            core = nullptr;
            return;
        }
        ...
        #if DEBUG
            // Set debug callback
            core->SetLogHook(discord::LogLevel::Warn, [](discord::LogLevel log, const char* message) { ASSERT_WARNING("Discord SDK failed: {0}", message); });
            core->SetLogHook(discord::LogLevel::Error, [](discord::LogLevel log, const char* message) { ASSERT_ERROR("Discord SDK failed: {0}", message); });
        #endif
        ...
        core->UserManager().OnCurrentUserUpdate.Connect([OnUserLoadedCallback]
        {
            ...
            core->ImageManager().Fetch(userIconHandle, true, [](const discord::Result result, const discord::ImageHandle handle)
            {
                ...
                else
                {
                    ...
                    core->ImageManager().GetData(handle, iconData.data(), static_cast<uint32>(iconData.size()));
