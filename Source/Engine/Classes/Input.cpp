...
namespace Sierra::Engine
{
    ...
    uint32 keyboardKeys[349];
    uint32 lastKeySet = 0;
    bool keySet = false;

    uint32 mouseButtons[349];
    uint32 lastButtonSet = 0;
    ...
    String UnicodePointToChar(uint32 unicodePoint);
    ...
    uint32 gamePadsConnected = 0;

    void RegisterGamePad(uint32 player);
    bool CheckGamePadConnection(uint32 player);

    /* --- POLLING METHODS --- */

    void Input::Initialize()
    {
        for (uint32 i = 0; i < MAX_GAME_PADS; i++)
        {
    ...
    void Input::Update()
    {
        ...
        for (uint32 i = MAX_GAME_PADS; i--;)
        {
            ...
            for (int32 j = 15; j--;)
            {
                uint32 oldState = gamePads[i].buttons[j];
                uint32 newState = gamePadState.buttons[j] + 1;
    ...
    void Input::SetGamePadMinimumStickSensitivity(const float minimumSensitivity, const uint32 player)
    {
    ...
    void Input::SetGamePadMinimumLeftStickSensitivity(const float minimumSensitivity, const uint32 player)
    {
        if (CheckGamePadConnection(player)) gamePads[player].minimumSensitivities[0] = minimumSensitivity;
    }

    void Input::SetGamePadMinimumRightStickSensitivity(const float minimumSensitivity, const uint32 player)
    {
    ...
    {
        return keyboardKeys[static_cast<uint32>(key)] == 2; // 2 = Press
    }

    bool Input::GetKeyHeld(const Key key)
    {
        return keyboardKeys[static_cast<uint32>(key)] == 3 || keyboardKeys[static_cast<uint32>(key)] == 2; // 3 = Repeat || 2 = Press
    }

    bool Input::GetKeyResting(const Key key)
    {
        return keyboardKeys[static_cast<uint32>(key)] == 0; // 0 = Rest
    }

    bool Input::GetKeyReleased(const Key key)
    {
        return keyboardKeys[static_cast<uint32>(key)] == 1; // 1 = Release
    }
    ...
    {
        return mouseButtons[static_cast<uint32>(button)] == 2; // 2 = Press
    }

    bool Input::GetMouseButtonHeld(const MouseButton button)
    {
        return mouseButtons[static_cast<uint32>(button)] == 3 || mouseButtons[static_cast<uint32>(button)] == 2; // 3 = Repeat || 2 = Press
    }

    bool Input::GetMouseButtonReleased(const MouseButton button)
    {
        return mouseButtons[static_cast<uint32>(button)] == 1; // 1 = Release
    }

    bool Input::GetMouseButtonResting(const MouseButton button)
    {
        return mouseButtons[static_cast<uint32>(button)] == 0; // 0 = Rest
    }
    ...
    String Input::GetGamePadName(uint32 player)
    {
        return gamePads[player].name;
    }

    bool Input::GetGamePadButtonPressed(const GamePadButton button, const uint32 player)
    {
        return CheckGamePadConnection(player) && gamePads[player].buttons[static_cast<uint32>(button)] == 2; // 2 - Press
    }

    bool Input::GetGamePadButtonHeld(const GamePadButton button, const uint32 player)
    {
        return CheckGamePadConnection(player) && (gamePads[player].buttons[static_cast<uint32>(button)] == 2 || gamePads[player].buttons[static_cast<uint32>(button)] == 3); // 2 - Press || 3 - Hold
    }

    bool Input::GetGamePadButtonReleased(const GamePadButton button, const uint32 player)
    {
        return CheckGamePadConnection(player) && gamePads[player].buttons[static_cast<uint32>(button)] == 1; // 1 - Press
    }

    bool Input::GetGamePadButtonResting(const GamePadButton button, const uint32 player)
    {
        return CheckGamePadConnection(player) && gamePads[player].buttons[static_cast<uint32>(button)] == 0; // 0 - Rest
    }

    Vector2 Input::GetGamePadLeftStickAxis(const uint32 player)
    {
        return CheckGamePadConnection(player) ? gamePads[player].axes[0] : Vector2(0, 0);
    }

    Vector2 Input::GetGamePadRightStickAxis(const uint32 player)
    {
        return CheckGamePadConnection(player) ? gamePads[player].axes[1] : Vector2(0, 0);
    }

    float Input::GetHorizontalGamePadLeftStickAxis(const uint32 player)
    {
        return CheckGamePadConnection(player) ? gamePads[player].axes[0].x : 0.0f;
    }

    float Input::GetVerticalGamePadLeftStickAxis(const uint32 player)
    {
        return CheckGamePadConnection(player) ? gamePads[player].axes[0].y : 0.0f;
    }

    float Input::GetHorizontalGamePadRightStickAxis(const uint32 player)
    {
        return CheckGamePadConnection(player) ? gamePads[player].axes[1].x : 0.0f;
    }

    float Input::GetVerticalGamePadRightStickAxis(const uint32 player)
    {
        return CheckGamePadConnection(player) ? gamePads[player].axes[1].y : 0.0f;
    }

    float Input::GetGamePadLeftTriggerAxis(const uint32 player)
    {
        return CheckGamePadConnection(player) ? gamePads[player].triggers[0] : 0.0f;
    }

    float Input::GetGamePadRightTriggerAxis(const uint32 player)
    {
        return CheckGamePadConnection(player) ? gamePads[player].triggers[1] : 0.0f;
    }

    bool Input::GetGamePadConnected(const uint32 player)
    {
    ...
    bool CheckGamePadConnection(uint32 player)
    {
        ...
        {
            ASSERT_WARNING("Game pad with an ID of [{0}] is not connected", player);
            return false;
        }
    ...
    void Input::KeyboardCharacterCallback([[maybe_unused]] GLFWwindow *windowPtr, const uint32 character)
    {
        enteredCharacters.push_back(UnicodePointToChar(character));
    }

    void Input::KeyboardKeyCallback([[maybe_unused]] GLFWwindow *windowPtr, const int32 keyCode, const int32 scanCode, int32 action, const int32 mods)
    {
        if (keyCode == static_cast<uint32>(Key::UNKNOWN)) return;

        keyboardKeys[static_cast<uint32>(keyCode)] = action + 1;
        lastKeySet = keyCode;
        keySet = true;
    }

    void Input::MouseButtonCallback([[maybe_unused]] GLFWwindow *windowPtr, const int32 button, const int32 action, const int32 mods)
    {
        mouseButtons[static_cast<uint32>(button)] = action + 1;
    ...
    void Input::MouseScrollCallback([[maybe_unused]] GLFWwindow *windowPtr, double xScroll, double yScroll)
    {
        ...
        scroll = { static_cast<float>(xScroll), static_cast<float>(yScroll) };
        scrollSet = true;
    }

    void Input::JoystickCallback(const int32 joystickID, const int32 connectionStatus)
    {
    ...
    void RegisterGamePad(const uint32 player)
    {
        gamePads[player].connected = true;
        gamePads[player].name = glfwGetGamepadName(static_cast<int32>(player));
    ...
    String UnicodePointToChar(const uint32 unicodePoint)
    {
